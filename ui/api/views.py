...
class SiteSettingsViewSet(viewsets.ModelViewSet):
    ...
    def post(self, request, pk):
        if pk == 'update':
            ...
            sett.save()
            try:
                Logs.objects.create(log_type='Update',log_category='Settings',log_message='Settings updated.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            except KeyError:
                pass
            return Response("Ok")
...
class ProfileViewSet(viewsets.ModelViewSet):
    ...
    def post(self, request, pk):
        if pk =='add':
            ...
            try:
                Logs.objects.create(log_type='Add',log_category='Profile',log_message='Added ' + pname,log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            except KeyError:
                pass
        elif pk == 'delete':
            ...
            p.delete()
            try:
                Logs.objects.create(log_type='Add',log_category='Profile',log_message='Delete ' + pname,log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            except KeyError:
                pass
        return Response("Ok")
...
class ProfileRadarrViewSet(viewsets.ModelViewSet):
    ...
    def post(self, request, pk):
        if pk == 'add':
            ...
            ret = pr.pk
            try:
                Logs.objects.create(log_type='Add',log_category='Radarr',log_message='Added ' + rt + ' to ProfileID ' + pid,log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            except KeyError:
                pass
        if pk == 'delete':
            ...
            ret = "DelOK"
            try:
                Logs.objects.create(log_type='Delete',log_category='Radarr',log_message='Deleted ' + rt + ' from ProfileID ' + pid,log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            except KeyError:
                pass
        return Response(ret)
...
class ProfileSonarrViewSet(viewsets.ModelViewSet):
    ...
    def post(self, request, pk):
        ...
        if pk == 'add':
            ...
            ret = ps.pk
            try:
                Logs.objects.create(log_type='Add',log_category='Sonarr',log_message='Sonarr entry added ' + ss.title + ' to ProfileID ' + pid,log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            except KeyError:
                pass
        if pk == 'delete':
            ...
            ret = "DelOK"
            try:
                Logs.objects.create(log_type='Delete',log_category='Sonarr',log_message='Sonarr entry deleted ' + ss.title + ' from ProfileID ' + pid,log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            except KeyError:
                pass
        return Response(ret)
...
class ProfileLidarrViewSet(viewsets.ModelViewSet):
    ...
    def post(self, request, pk):
        if pk == 'add':
            ...
            try:
                Logs.objects.create(log_type='Add',log_category='Lidarr',log_message='Lidarr entry added',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            except KeyError:
                pass
        if pk == 'delete':
            ...
            pl.delete()
            try:
                Logs.objects.create(log_type='Delete',log_category='Lidarr',log_message='Lidarr entry added',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            except KeyError:
                pass
        return Response("Ok")
...
def RunSync(request):
    ...
    try:
        Logs.objects.create(log_type='Sync',log_category='System',log_message='Sync initiated for Profile ' + prof.profile_name,log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
    except KeyError:
        pass
    copyTheFile(idList, destDir, prof_id)
    try:
        Logs.objects.create(log_type='Sync',log_category='System',log_message='Sync completed for Profile ' + prof.profile_name,log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
    except KeyError:
        pass
    try:
...
def marksynced(request):
    try:
        ...
        try:
            Logs.objects.create(log_type='Sync',log_category='System',log_message='Bulk override of Sync Date initiated.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
        except:
            pass
        prof_id = request.POST.get('prof_id')
        #prof = Profile.objects.get(id=prof_id)
        ...
        for pr in prlist:
            pr.lastRun = datetime.utcnow().strftime("%b %d %Y %H:%M:%S")
            pr.save()
        try:
            Logs.objects.create(log_type='Sync',log_category='System',log_message='Bulk override of Sync Date completed.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
        except:
            pass
    except:
        pass
...
def markmoviesmonitored(request):
    try:
        ...
        try:
            Logs.objects.create(log_type='Sync',log_category='System',log_message='Bulk override, mark all Monitored for Profile (' + prof.profile_name + ') initiated.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
        except:
            pass
        ...
        try:
            if isSuccessfull:
                prof = Profile.objects.get(id=prof_id)
                prof.profile_lastRun = datetime.utcnow().strftime("%b %d %Y %H:%M:%S")
                prof.save()
                Logs.objects.create(log_type='Sync',log_category='System',log_message='Bulk override, mark all Monitored for Profile (' + prof.profile_name + ') completed.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            else:
                Logs.objects.create(log_type='Sync',log_category='System',log_message='Bulk override, mark all Monitored for Profile (' + prof.profile_name + ') FAILED.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
        except:
            pass
    except:
        pass
...
def automonitor(request):
    ...
    try:
        ...
        if runType=='enable':
            ...
            prof.save()
            response = "OK"
            Logs.objects.create(log_type='System',log_category='System',log_message='Automonitor new movies for Profile (' + prof.profile_name + ') enabled.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
        elif runType=='disable':
            prof_id = request.POST.get('prof_id')
            prof = Profile.objects.get(id=prof_id)
            ...
            prof.save()
            response = "OK"
            Logs.objects.create(log_type='System',log_category='System',log_message='Automonitor new movies for Profile (' + prof.profile_name + ') disabled.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
    except:
        pass
    return Response(response)
...
def changeRadarrStatus(request):
    ...
    try:
        ...
        if runType=='enable':
            ...
            response = "OK"
            Logs.objects.create(log_type='System',log_category='Radarr',log_message='Radarr Connection Status changed - Connected.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
        elif runType=='disable':
            settings.isConnected = False
            response = "OK"
            Logs.objects.create(log_type='System',log_category='Radarr',log_message='Radarr Connection Status changed - Disconnected.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
    except:
        pass
    return Response(response)
...
def changeSonarrStatus(request):
    ...
    try:
        ...
        if runType=='enable':
            ...
            response = "OK"
            Logs.objects.create(log_type='System',log_category='Sonarr',log_message='Sonarr Connection Status changed - Connected.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
        elif runType=='disable':
            settings.isSonarrConnected = False
            response = "OK"
            Logs.objects.create(log_type='System',log_category='Sonarr',log_message='Sonarr Connection Status changed - Disconnected.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
    except:
        pass
    return Response(response)
    ...
    try:
        ...
        if runType=='skip':
            ...
            if ProfileSonarrEpisode.objects.filter(profile_id=prof_id,sonarr_id=sid).count() == 1:
                ...
                ProfileSonarrEpisode.objects.create(profile_id=prof_id,sonarr_id=sid,lastRun=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            response = "OK"
            try:
                Logs.objects.create(log_type='Add',log_category='Sonarr',log_message='Sonarr ' + ser.title + '[' + str(sem.episodeNumber) + ']' + sem.title + ' skipped for ' + prof.profile_name + '.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            except:
                pass
        elif runType=='unskip':
            ...
            response = "OK"
            try:
                Logs.objects.create(log_type='Add',log_category='Sonarr',log_message='Sonarr ' + ser.title + '[' + sem.episodeNumber + ']' + sem.title + ' un-skipped for profile ' + prof.profile_name + '.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
            except:
                pass
    except Exception as e:
        try:
            Logs.objects.create(log_type='ERROR',log_category='Sonarr',log_message='Sonarr skipEpisode error [' + e + '.',log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
        except:
            pass
        pass
    return Response(response)
...
def RunSyncShows(request):
    ...
    try:
        Logs.objects.create(log_type='Sync',log_category='System',log_message='Sonarr Sync initiated for Profile ' + prof.profile_name,log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
    except KeyError:
        pass
    copyTheShow(idList, destDir, prof_id, create_show_fldr, create_season_fldr)
    try:
        Logs.objects.create(log_type='Sync',log_category='System',log_message='Sonarr Sync completed for Profile ' + prof.profile_name,log_datetime=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
    except KeyError:
        pass
    try:
        prof.profile_lastPath = destDir
        ...
        for pss in profile_sonarr_show_list:
            ...
            for se in sel:
                ...
                if ProfileSonarrEpisode.objects.filter(profile_id=prof_id,sonarr_id=se.sonarr_id).count() == 1:
                    ...
                    pse = ProfileSonarrEpisode.objects.create(profile_id=prof_id,sonarr_id=se.sonarr_id,lastRun=datetime.utcnow().strftime("%b %d %Y %H:%M:%S"))
